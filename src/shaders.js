...
/* The tunables from the config become #defines, so the numbers live in one
...
/* The only export: hand it a config, get back the sources with that config's
   numbers baked in. Called once per renderer, so a setConfig() that touches
   glsl/glslInt can rebuild the programs against new values. */
